...
public class MainPanel extends JPanel {
    ...
    public int getNumNeighborsOld(int x, int y) {
	int size = _size;
	int leftX = (x - 1) % size;
	int rightX = (x + 1) % size;
	int upY = (y - 1) % size;
	...
	if (_cells[x][upY].getAlive())        { numNeighbors++; }
	if (_cells[x][downY].getAlive())      { numNeighbors++; }
	 // Remove the unnecessary convertToInt() method since the numNeighbors is already an Integer  
	return numNeighbors; 
    ...
    // New backup method, getting rid of the unnecessary backup cell define and initialize
    // Move them to the MainPanel construct method, so that it will only be executed once
    public void backup() {
	for (int j = 0; j < _size; j++) {
	    for (int k = 0; k < _size; k++) {		
		_backupCells[j][k].setAlive(_cells[j][k].getAlive());
	...
	while (_running) {
	    System.out.println("Running...");
            ...
            // Get rid of the unnecessary part to speed up the program
...
//	    _r = origR;
	...
	setLayout(new GridLayout(size, size));
        // Move the backup cells definition to here, so that the backup()
        // doesn't need to create it every time
        // This will save the conputer resources
        _backupCells = new Cell[size][size];
	_cells = new Cell[size][size];
        
	for (int j = 0; j < size; j++) {
	    for (int k = 0; k < size; k++) {
                // Move the backup cells initialization to here, so that the backup()
                // doesn't need to initialize it every time
                // This will save the conputer resources
                _backupCells[j][k] = new Cell();
		_cells[j][k] = new Cell();
		this.add(_cells[j][k]);
